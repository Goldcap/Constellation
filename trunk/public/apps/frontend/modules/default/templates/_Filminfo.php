...
<?php if (count($film["film_link"])) {?>          
<p><strong>Other Links</strong>
<?php foreach ($film["film_link"] as $auser) {echo linkSplit($auser)."<br />";}?>
</p>         
<?php }?> 
...
<?php } else {?>
<h4 class="film_info">This Week's Film</h4>
<div class="movie_poster">
<img class="poster" src="/uploads/screeningResources/<?php echo $film["film_id"];?>/logo/poster<?php echo $film["film_logo"];?>" alt="<?php echo $film["film_name"];?>" class="widget_video_still"  border="0"/>	</div>
<?php echo preg_replace("/^<p>/","<p class='friggin_squished'>",WTVRCleanString($film["program_info"]));?></p>
