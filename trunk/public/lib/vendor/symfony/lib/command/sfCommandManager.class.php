...
class sfCommandManager
{
    ...
    foreach ($this->argumentSet->getArguments() as $argument)
    {
      if (array_key_exists($position, $this->parsedArgumentValues))
      {
        ...
        else
        {
          ...
          //Added this, override OPTIONS with ARGUMENTS
          $this->overrideOption($argument->getName(),$this->parsedArgumentValues[$position]);
  ...
  /**
   * Override Option with an Argument (by name).
   *
   * @param string $argument The option argument
   */
  protected function overrideOption($name, $value)
  {
   if ( $this->optionSet->hasOption($name))
      {
