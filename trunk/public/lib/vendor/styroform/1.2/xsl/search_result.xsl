...
<xsl:template match="LIST" mode="search_result">
    ...
    <div id="results">
      ...
      <table border="1">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:apply-templates select="self::*" mode="search_result_table" />
          </xsl:otherwise>
        </xsl:choose>
      </table>
...
<xsl:template match="LIST" mode="search_result_table">
  
  <table cellpadding="4">
    
    <xsl:apply-templates select="LISTITEM" mode="search_result" />
    
  </table>
  
</xsl:template>

<xsl:template match="LISTITEM" mode="search_result">
  
  <xsl:variable name="styrogroup" select="cms_object_keyword" />
  <xsl:if test="count(preceding-sibling::LISTITEM[cms_object_keyword = $styrogroup]) = 0">
    <tr>
      <td>
      <span class="column_Group"><xsl:value-of select="cms_object_keyword" /></span>
      </td>
    </tr>
  </xsl:if>
  
  <tr>
    <td><a href="{../@script}/{cms_object_link}"><xsl:value-of select="cms_object_title" disable-output-escaping="yes" /></a></td>
  </tr>
