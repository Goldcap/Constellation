...
<xsl:template match="FORMELEMENT[TYPE='errornotice']">
  ...
  <xsl:choose>
    <xsl:when test="DEFAULT/@visible = 'true'">
      <span id="errornotice_{$THEFORMNAME}" class="errornotice_visible"><xsl:value-of select="DEFAULT" /></span>
    </xsl:when>
    <xsl:otherwise>
		  <span id="errornotice_{$THEFORMNAME}" class="errornotice"><xsl:value-of select="DEFAULT" /></span>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
