...
<xsl:template name="initCapSpace">
  <xsl:param name="str" />
  <xsl:choose>
    <xsl:when test="contains($str,' ')">
      <xsl:call-template name="initcaps"><xsl:with-param name="x" select="substring-before($str,' ')" /></xsl:call-template><xsl:text> </xsl:text><xsl:call-template name="initCap"><xsl:with-param name="str" select="substring-after($str,' ')" /></xsl:call-template>
    </xsl:when>
    <xsl:otherwise>
      <xsl:call-template name="initcaps"><xsl:with-param name="x" select="$str" /></xsl:call-template>
    </xsl:otherwise>
