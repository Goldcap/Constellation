...
<xsl:template match="LIST" mode="WT_ALBUM">
    ...
    <table width="500" cellpadding="20" border="0">
     <xsl:choose>
        <xsl:when test="count(LISTITEM) = 0">
          ...
          <table cellpadding="4">
            <tr>
              <td colspan="2">Sorry, No Albums Available.</td>
            </tr>
          </table>
          </td>
          </tr>
        ...
        <xsl:otherwise>
          <xsl:apply-templates select="self::*" mode="wtalbum_image_table" />
        </xsl:otherwise>
      </xsl:choose>
...
<xsl:template match="LIST" mode="wtalbum_image_table">
    
  <xsl:for-each select="LISTITEM[position() mod 4 = 1]">
    <tr height="130">
    <xsl:apply-templates select=".|following-sibling::LISTITEM[position() &lt; 4]" mode="wt_album">
...
<xsl:template match="LISTITEM" mode="wt_album">
