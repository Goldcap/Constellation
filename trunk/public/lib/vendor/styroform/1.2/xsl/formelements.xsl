...
<xsl:variable name="monthlocation">../xml/months.xml</xsl:variable>
<xsl:variable name="minutelocation">../xml/minutes.xml</xsl:variable>
<xsl:variable name="countrylocation">../xml/countries.xml</xsl:variable>
<xsl:variable name="continentlocation">../xml/continents.xml</xsl:variable>
<xsl:variable name="statelocation">../xml/states.xml</xsl:variable>
...
<xsl:template name="REQS">
	<xsl:if test="//JSVALIDATE != 'FALSE'">
		<xsl:if test="VTIP != 'omit'">
			...
			<span id="error{NAME}" class="errorhidden">false</span>
		</xsl:if>
	</xsl:if>
</xsl:template>

<xsl:template match="FORMELEMENT[TYPE='display']">
	<span class="{DISPLAYCLASS} display" id="{NAME}">
	 <xsl:choose>
		 <!-- What the heck is this? Doesn't look finished, eh? -->
     <xsl:when test="../TYPE/@nullomit = 'true'">
      <xsl:apply-templates select="DISPLAY" disable-output-escaping="yes" />
	   </xsl:when>
	   <xsl:otherwise>
      <xsl:apply-templates select="DISPLAY" disable-output-escaping="yes" />
	   </xsl:otherwise>
...
<xsl:template match="FORMELEMENT[TYPE='selectdisplay']">
  <xsl:apply-templates select="DISPLAY" />
  <span class="{CLASS/@wrapper} displayselect" id="{../../NAME}">
	<xsl:for-each select="VALUES/VALUE[@selected='TRUE']">
   	<xsl:value-of select="self::*" disable-output-escaping="yes" /><br />
	</xsl:for-each>
	<xsl:for-each select="VALUE[@selected='TRUE']">
   	<xsl:value-of select="self::*" disable-output-escaping="yes" /><br />
	</xsl:for-each>
	</span>
</xsl:template>

<xsl:template match="FORMELEMENT[TYPE='displayselect'][count(VALUES/VALUE[@selected='TRUE']) = 1]">
	<xsl:variable name="THEDEF">
	 <xsl:choose>
	...
	<xsl:variable name="THEVAL">
	  <xsl:choose>
	   <xsl:when test="VALUES/VALUE[@id=$THEDEF] != ''"><xsl:value-of select="VALUES/VALUE[@id=$THEDEF]" /></xsl:when>
	   <xsl:when test="VALUE[@id=$THEDEF] != ''"><xsl:value-of select="VALUE[@id=$THEDEF]" /></xsl:when>
	   <xsl:when test="VALUES/VALUE[@value=$THEDEF] != ''"><xsl:value-of select="VALUES/VALUE[@value=$THEDEF]" /></xsl:when>
	   <xsl:when test="VALUE[@value=$THEDEF] != ''"><xsl:value-of select="VALUE[@value=$THEDEF]" /></xsl:when>
   </xsl:choose>
	</xsl:variable>
  <span class="{DISPLAYCLASS} displayselect" id="{NAME}">
	 <xsl:choose>
		...
		<xsl:otherwise><xsl:value-of select="$THEVAL" disable-output-escaping="yes" /></xsl:otherwise>
	 </xsl:choose>
	</span>
</xsl:template>

<xsl:template match="FORMELEMENT[TYPE='displaycheckradio']">
  <xsl:apply-templates select="DISPLAY" />
  <span class="{CLASS/@wrapper} displayselect" id="{../../NAME}">
	<xsl:for-each select="VALUES/VALUE[@selected='TRUE']">
	  <xsl:value-of select="self::*" disable-output-escaping="yes" /><br />
	</xsl:for-each>
	<xsl:for-each select="VALUEGROUP/VALUE[@selected='TRUE']">
	  <xsl:value-of select="self::*" disable-output-escaping="yes" /><br />
	</xsl:for-each>
	</span>
</xsl:template>

<xsl:template match="FORMELEMENT[TYPE='displaydb']">
  <xsl:apply-templates select="DISPLAY" />
  <span class="{CLASS/@wrapper} displayselect" id="{../../NAME}">
	<xsl:for-each select="VALUES/VALUE[@selected='TRUE']">
	  <xsl:value-of select="self::*" disable-output-escaping="yes" /><br />
...
<xsl:template match="FORMELEMENT[TYPE='password']">
	...
	<xsl:apply-templates select="DISPLAY" />
		<input type="password" name="{NAME}" class="{$THECLASS}" MAXLENGTH="{SIZE}" id="{NAME}" value="{DEFAULT}" vtip="{VTIP}" vreq="{REQUIRED}" autocomplete="off" />
...
<xsl:template match="FORMELEMENT[TYPE='textarea']">
	...
	<xsl:apply-templates select="DISPLAY" />
	  ...
	  <xsl:when test="SIZE != ''">
		...
		<xsl:if test="CLASS='inputareadojo'">
    <a href="#" onclick="toggleEditor('{NAME}')">Toggle Editor</a>
    </xsl:if>
    <xsl:call-template name="REQS" />
	</span>
</xsl:template>
...
<xsl:template match="FORMELEMENT[TYPE='displaydate']">
	<xsl:variable name="MONTHS" select="document($monthlocation)/VALUES" />
  ...
  <span class="{DISPLAYCLASS}">
	...
	<xsl:variable name="SELECTEDVALUE" select="DEFAULT/@month" />
  <xsl:for-each select="$MONTHS/VALUE">
	<xsl:if test="number($SELECTEDVALUE) = number(@id)">
	...
	<xsl:text> </xsl:text>
	<xsl:variable name="SELECTEDDAY" select="DEFAULT/@day" />
  <xsl:value-of select="number($SELECTEDDAY)" />,
  <xsl:text> </xsl:text>
	
  <xsl:variable name="SELECTEDYEAR" select="DEFAULT/@year" />
	<xsl:value-of select="number($SELECTEDYEAR)"/>
...
<xsl:template match="FORMELEMENT[TYPE='numericmonthyear']">
	<xsl:variable name="MONTHS" select="document($monthlocation)/VALUES" />
      ...
      <xsl:for-each select="$MONTHS/VALUE">
    	<xsl:choose>
		  <xsl:when test="$SELECTEDVALUE = @data">
    		...
    		<xsl:otherwise>
    			<option value="{@id}"><xsl:value-of select="@id" /> - <xsl:value-of select="@abbr" /> </option>
    		</xsl:otherwise>
    	</xsl:choose>
    </xsl:for-each>
	</select>
...
<xsl:template match="FORMELEMENT[TYPE='country_extended']">
	<xsl:variable name="THECOUNTRIES" select="document($countrylocation)/THECOUNTRIES"/>
	...
	<select id="{NAME}" class="{$THECLASS}" name="{NAME}"  title="{TITLE}">
		...
		<xsl:for-each select="$THECOUNTRIES/ACOUNTRY">
        	<xsl:choose>
				<xsl:when test="$SELECTEDVALUE = @abbr">
        			<option value="{@abbr}" selected="selected"><xsl:value-of select="@name" /></option>
        		</xsl:when>
        		<xsl:otherwise>
        			<option value="{@abbr}"><xsl:value-of select="@name" /></option>
        		</xsl:otherwise>
        	</xsl:choose>
        </xsl:for-each>
	</select>
...
<xsl:template match="FORMELEMENT[TYPE='continent']">
	<xsl:variable name="THECONTINENTS" select="document($continentlocation)/THECONTINENTS"/>
	...
	<select id="{NAME}" class="{$THECLASS}" name="{NAME}"  title="{TITLE}">
		<option value="0" selected="selected"><xsl:value-of select="DEFAULT" /></option>
		<xsl:variable name="SELECTEDVALUE" select="DEFAULT/@selected" />
		<xsl:for-each select="$THECONTINENTS/ACONTINENT">
        	<xsl:choose>
				<xsl:when test="$SELECTEDVALUE = @abbr">
        			<option value="{@abbr}" selected="selected"><xsl:value-of select="@abbr" /></option>
        		</xsl:when>
...
<xsl:template match="FORMELEMENT[TYPE='countrydisplay']">
  <xsl:apply-templates select="DISPLAY" />
  <span class="{CLASS/@wrapper} displayselect" id="{../../NAME}">
	<xsl:value-of select="DEFAULT/@selected" disable-output-escaping="yes" /><br />
	</span>
</xsl:template>
...
<xsl:template match="FORMELEMENT[TYPE='datetimepicker']">
  <input type="text" id="{NAME}" name="{NAME}" class="{CLASS}" value="{DEFAULT}" />
    <button id="{NAME}_trigger">...</button>
    <script type="text/javascript">
      var <xsl:value-of select="NAME" />_cal = Zapatec.Calendar.setup({
        firstDay          : 1,
        showOthers        : true,
        showsTime         : true,
        timeFormat        : "12",
        ...
        button            : "<xsl:value-of select="NAME" />_trigger",
...
<xsl:template match="FORMELEMENT[TYPE='datepicker']">
  <input type="text" id="{NAME}" name="{NAME}" class="{CLASS}" value="{DEFAULT}" />
    <button id="{NAME}_trigger">...</button>
    <script type="text/javascript">
      var <xsl:value-of select="NAME" />_cal = Zapatec.Calendar.setup({
...
<xsl:template match="FORMELEMENT[TYPE='select']">
   ...
   <xsl:variable name="DEFAULTVALUE">
    <xsl:choose>
      <xsl:when test="DEFAULT/@value != ''"><xsl:value-of select="DEFAULT/@value" /></xsl:when>
      <xsl:otherwise>0</xsl:otherwise>
    </xsl:choose>
   </xsl:variable>
   <xsl:choose>
      <xsl:when test="((count(DEFAULTVALUES/VALUE) = 0) and (TYPE/@multiple>0))">
   	    <option value="{$DEFAULTVALUE}" selected="selected"><xsl:value-of select="DEFAULT" /></option>
		  </xsl:when>
		  <xsl:when test="DEFAULT/@selected = ''">
   	    <option value="{$DEFAULTVALUE}" selected="selected"><xsl:value-of select="DEFAULT" /></option>
		  </xsl:when>
		  <xsl:otherwise>
		    <option value="{$DEFAULTVALUE}"><xsl:value-of select="DEFAULT" /></option>
		  </xsl:otherwise>
		</xsl:choose>
...
<xsl:template match="FORMELEMENT[TYPE='checkdb']">
	<span class="{DISPLAYCLASS}">
	<xsl:apply-templates select="REQUIRED" />
	<xsl:apply-templates select="DISPLAY" />
...
<xsl:template match="VALUE" mode="checkradio">
  <xsl:variable name="THENAME">
	...
	<xsl:variable name="THETYPE">
	  <xsl:choose>
	   <xsl:when test="../../TYPE='radiodb'">radio</xsl:when>
	   <xsl:when test="../../TYPE='checkdb'">checkbox</xsl:when>
  ...
  <xsl:variable name="THEVALUE">
	...
	<span class="{$THECLASS}">
		<xsl:if test="$THEVALUE != 'null'">
			<xsl:element name="input">
				<xsl:attribute name="type"><xsl:value-of select="$THETYPE" /></xsl:attribute>
				...
				<xsl:if test="$SELECTED = 'TRUE'"><xsl:attribute name="checked">checked</xsl:attribute><xsl:attribute name="selected">selected</xsl:attribute></xsl:if>
			</xsl:element>
		</xsl:if>
		<xsl:choose>
...
<xsl:template match="FORMELEMENT[TYPE='submitimage']">
		<span class="{DISPLAYCLASS}">
		  <button class="custombutton {CLASS}" type="submit" name="SUBMIT_{NAME}" id="SUBMIT_{NAME}" onclick="{DEFAULT/@onclick};setAction(this,'{NAME}');" value="{NAME}" ><img src="{DEFAULT}" alt="{TITLE}" class="{DISPLAYCLASS}" /></button>
   	</span>
</xsl:template>

<xsl:template match="FORMELEMENT[TYPE='resetimage']">
		<span class="{DISPLAYCLASS}">
			<button class="custombutton {CLASS}" type="reset" name="SUBMIT_{NAME}" onclick="document.forms['{//FORMNAME}'].reset();return false;" value="{NAME}" ><img src="{DEFAULT}" alt="reset" class="{DISPLAYCLASS}"/></button>
   	</span>
</xsl:template>
...
<xsl:template match="FORMELEMENT[TYPE='submit']">
		<span class="{DISPLAYCLASS}">
		<xsl:choose>
    <xsl:when test="DEFAULT/@onclick != ''">
      <input type="submit" onclick="{DEFAULT/@onclick}" id="SUBMIT_{NAME}" name="SUBMIT_{NAME}" class="{CLASS}" value="{DEFAULT}" />
		</xsl:when>
    <xsl:otherwise>
...
<xsl:template match="FORMELEMENT[TYPE='image']">
    ...
    <xsl:when test="DEFAULT/@uri != ''">
      <span class="{DISPLAYCLASS}">
			 <a href="{DEFAULT/@uri}"><img src="{$PREPATH}{DEFAULT}" onclick="{DEFAULT/@onclick}" onmouseover="{DEFAULT/@onmouseover}" name="{NAME}" id="{NAME}" class="{CLASS}" border="0" width="{$WIDTH}" height="{$HEIGHT}" /></a>
		  </span>
    </xsl:when>
    <xsl:when test="DEFAULT != ''">
    <span class="{DISPLAYCLASS}">
			<img src="{$PREPATH}{DEFAULT}" name="{NAME}" id="{NAME}" class="{CLASS}" onclick="{DEFAULT/@onclick}" onmouseover="{DEFAULT/@onmouseover}" border="0" width="{$WIDTH}" height="{$HEIGHT}" />
		</span>
    </xsl:when>
    <xsl:otherwise>
      <span class="{DISPLAYCLASS}">
...
<xsl:template match="FORMELEMENT[TYPE='iterator']">
    <!--<xsl:choose>
    <xsl:when test="DEFAULT/@uri != ''">
      <span class="{DISPLAYCLASS}">
...
<xsl:template match="FORMELEMENT[TYPE='errornotice']">
  <xsl:variable name="THEFORMNAME">
    <xsl:choose>
      <xsl:when test="../../FORMNAME != ''"><xsl:value-of select="../../FORMNAME" /></xsl:when>
      <xsl:when test="../../../FORMNAME != ''"><xsl:value-of select="../../../FORMNAME" /></xsl:when>
  ...
  <xsl:choose>
    <xsl:when test="DEFAULT/@visible = 'true'">
      <span id="errornotice_{$THEFORMNAME}" class="errornotice_visible"><xsl:value-of select="DEFAULT" disable-output-escaping="yes" /></span>
    </xsl:when>
    <xsl:otherwise>
		  <span id="errornotice_{$THEFORMNAME}" class="errornotice"><xsl:value-of select="DEFAULT" disable-output-escaping="yes" /></span>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template match="FORMELEMENT[TYPE='hidden']">
  <xsl:variable name="thevalue" disable-output-escaping="yes"><xsl:value-of select="DEFAULT" /></xsl:variable>
		<span class="{DISPLAYCLASS}">
			<input id="{NAME}" type="hidden" value="{$thevalue}" name="{NAME}" class="{CLASS}" border="0" />
		</span>
</xsl:template>

<xsl:template match="FORMELEMENT[TYPE='container']">
  <xsl:variable name="thevalue" disable-output-escaping="yes"><xsl:value-of select="DEFAULT" /></xsl:variable>
	<div class="reqs styro_container">
    	<span id="container_{NAME}" name="container_{NAME}_{//FORMNAME}" class="reqs" style="display: none;" border="0"><xsl:value-of select="$thevalue" /></span>
  </div>
</xsl:template>
