...
class LogAdminSearch_format_utility {
  ...
  function getConf( $xmlObj ) {
    ...
    if ($this -> date) {
    	$this -> map -> createSingleElementByPath("criteria","//criteria[@column='flashlog_message']",array("scope"=>"PROCESS","value"=>$this -> date,"column"=>"flashlog_date","type"=>"daterange"));
    }
    
    //if ($this -> errors) {
    //	$this -> map -> createSingleElementByPath("criteria","//criteria[@column='flashlog_message']",array("value"=>1,"column"=>"flashlog_error","type"=>"integer"));
    //}
    
    if (! $this -> term) {
      $this -> map -> removeSingleElementByPath("//criteria[@column='flashlog_ticket']");
      $this -> map -> removeSingleElementByPath("//criteria[@column='flashlog_message']");
    }
  ...
  function formatFlashError($value) {
    switch($value) {
     case -1:
      ...
      break;
      case 0:
      return '<span style="color: black">Info</span>';
      break;
     case 1:
      ...
      break;
      case 3:
      return '<span style="color: #00FF00">Fatal</span>';
      break;
    }
  }
