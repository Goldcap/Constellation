...
function addFilter( element ) {
  switch (element) {
    ...
    case "country":
      ...
      break;
     case "region":
      if ($("#film_geoblocking_region").val() != '') {
      ...
      break;
     case "city":
      if ($("#film_geoblocking_city").val() != 0) {
        html = '<div id="'+$("#film_geoblocking_city").val().replace(' ','')+'"><span class="gbfilter">'+$("#film_geoblocking_city").val()+'</span><span><img src="/images/Neu/16x16/actions/edit-undo.png" onclick="removeFilter(\''+$("#film_geoblocking_city").val().replace(' ','')+'\')" /></span></div>';
      } else {
        return false;
      }
      break;
