...
package org.osmf.smil.loader
{
	...
	public class SMILLoader extends LoaderBase
	{
		...
		private var smilInMemory:Boolean = false;
		/**
		 * Constructor.
		 * 
		...
		override public function canHandleResource(resource:MediaResourceBase):Boolean
		{
			...
			else if (resource is URLResource && resource.mediaType!="loadedSMIL")
			{
			...
			}else if(resource.mediaType=="loadedSMIL"){
				canHandle = true
				smilInMemory = true;
			}
		...
		{			
			...
			}else if(this.smilInMemory==true){
				var parser:SMILParser = createParser();
				var smilDocument:SMILDocument = parser.parse(URLResource(loadTrait.resource).url);
				finishLoad(loadTrait, smilDocument);
			}
		}
