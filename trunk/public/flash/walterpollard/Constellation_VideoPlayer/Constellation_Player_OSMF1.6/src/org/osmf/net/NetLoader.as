		...
		CONFIG::FLASH_10_1	
		{
			...
			private function setupStreamReconnect(loadTrait:NetStreamLoadTrait):void
			{
				...
				function onNetStatus(event:NetStatusEvent):void
				{
					...
					switch(event.info.code)
					{
						case NetConnectionCodes.CONNECT_SUCCESS:
							...
							CONFIG::LOGGING
							{			
								logger.info(STREAM_RECONNECT_LOGGING_PREFIX+"closing the old (bad) NetConnection");
							}							
						...
						case NetConnectionCodes.CONNECT_FAILED:
							...
							{	
								if (loadTrait.netStream != null)
								{
									logger.info(STREAM_RECONNECT_LOGGING_PREFIX+"connection failed, bufferLength is "+loadTrait.netStream.bufferLength);
								}
								else
								{
									logger.info(STREAM_RECONNECT_LOGGING_PREFIX+"connection failed, bufferLength is zero");
								}
							}
						...
						case NetStreamCodes.NETSTREAM_BUFFER_EMPTY:
							CONFIG::LOGGING
							{			
								logger.info(STREAM_RECONNECT_LOGGING_PREFIX+"buffer empty, netConnection.connected="+netConnection.connected);
							}
				...
				function onTimeoutTimer(event:TimerEvent):void
				{
					CONFIG::LOGGING
					{			
						logger.info(STREAM_RECONNECT_LOGGING_PREFIX+"reconnect timer timed out...");
					}
					reconnectHasTimedOut = true;	
				}
				...
				function onReconnectTimer(event:TimerEvent):void
				{
					...
					if (netConnection === loadTrait.connection)
					{
						...
						CONFIG::LOGGING
						{
							logger.info(STREAM_RECONNECT_LOGGING_PREFIX+"About to create a new NetConnection...");
						}
