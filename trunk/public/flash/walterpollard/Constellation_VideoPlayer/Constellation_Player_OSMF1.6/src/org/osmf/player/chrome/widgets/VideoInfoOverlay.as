...
package org.osmf.player.chrome.widgets
{
	...
	{	
		...
		protected function onDebugMessage(event:constellationEvent):void
		{
			...
			if(this._debugTextField.length>3500){
	...
	//	private var closeButton:CloseButton;
		...
		/*	if (container.width < videoInfoOverlay.width || containerWidth != container.width)
			...
			videoInfo += "\nDetected Video Type \t"+ ExternalConfig.getInstance().videoDataType+"\n";
			...
			if (media && media.hasTrait(MediaTraitType.LOAD))
			{
				...
				if (loadTrait)
				{
					...
					if (this._netStream || this._akamaiZStream)
					{
						...
						var bandWidth:String = this.getBandWidth();
						...
						videoInfo += "Frame rate:\t" 
							...
							+ " fps\n"
							+ "\t";
						videoInfo += "Frame rate @ 0 count:\t" 
							+ ExternalConfig.getInstance().zeroFPSCount
							+ "\t   Max Limit: "+ExternalConfig.getInstance().zeroFPSLimit
							+ "\n";
...
//						*bytesPushedToPeers
		...
		private function getBandWidth():String
		{
			...
			if(this._netStream){
				...
				bandwidth = this._akamaiZStream.bandwidth.toString()
			}
			}catch(err:Error){
				bandwidth = "N\A - N\R";
			}
		...
		private function getStreamName():String
		{
			...
			try{
				if(this.target.streamSMILParser.dsi!=null){
					...
					if(this._akamaiZStream.manifest==null){
						streamName = "null";	
					}else{
