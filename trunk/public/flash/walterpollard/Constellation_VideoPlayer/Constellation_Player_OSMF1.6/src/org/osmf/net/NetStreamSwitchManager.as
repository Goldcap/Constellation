...
package org.osmf.net
{
	...
	public class NetStreamSwitchManager extends NetStreamSwitchManagerBase
	{
		...
		override public function set autoSwitch(value:Boolean):void
		{
			...
			CONFIG::LOGGING
			{
				logger.info("autoSwitch() - setting to " + value);
			}
			...
			if (_autoSwitch)
			{
				CONFIG::LOGGING
				{
					logger.info("autoSwitch() - starting check rules timer.");
				}
			...
			else
			{
				CONFIG::LOGGING
				{
					logger.info("autoSwitch() - stopping check rules timer.");
				}
		...
		override public function switchTo(index:int):void
		{
			if (!_autoSwitch)
			{
				...
				else
				{
					CONFIG::LOGGING
					{
						logger.info("switchTo() - manually switching to index: " + index);
					}
		...
		protected function canAutoSwitchNow(newIndex:int):Boolean
		{
			...
			if (dsiFailedCounts[newIndex] >= 1)
			{
				...
				if (current - failedDSI[newIndex] < DEFAULT_WAIT_DURATION_AFTER_DOWN_SWITCH)
				{
					CONFIG::LOGGING
					{
						logger.info("canAutoSwitchNow() - ignoring switch request because index has " + dsiFailedCounts[newIndex]+" failure(s) and only "+ (current - failedDSI[newIndex])/1000 + " seconds have passed since the last failure.");
					}
					return false;
				}
			}
		...
		private function executeSwitch(targetIndex:int):void 
		{
			...
			CONFIG::LOGGING
			{
				logger.info("executeSwitch() - Switching to index " + (targetIndex) + " at " + Math.round(dsResource.streamItems[targetIndex].bitrate) + " kbps");
			}
		...
		private function checkRules(event:TimerEvent):void 
		{
			   ...
			   ) 
			{
				...
				{				
					CONFIG::LOGGING
					{
						logger.info("checkRules() - Calling for switch to " + newIndex + " at " + dsResource.streamItems[newIndex].bitrate + " kbps");
					}
		...
		private function onNetStatus(event:NetStatusEvent):void
		{
			CONFIG::LOGGING
			{
				logger.info("onNetStatus() - event.info.code=" + event.info.code);
			}
			...
			switch (event.info.code) 
			{
				...
				case NetStreamCodes.NETSTREAM_PLAY_STOP:
					...
					CONFIG::LOGGING
					{
					logger.info("onNetStatus() - Stopping rules since server has stopped sending data");
					}
					break;
			}			
		}
		...
		private function onPlayStatus(info:Object):void
		{
			CONFIG::LOGGING
			{
				logger.info("onPlayStatus() - info.code=" + info.code);
			}
			...
			switch (info.code)
			{
				case NetStreamCodes.NETSTREAM_PLAY_TRANSITION_COMPLETE:
					...
					CONFIG::LOGGING
					{
					logger.info("onPlayStatus() - Transition complete to index: " + currentIndex + " at " + Math.round(dsResource.streamItems[currentIndex].bitrate) + " kbps");
					}
		...
		private function prepareForSwitching():void
		{
			...
			CONFIG::LOGGING
			{
				logger.info("prepareForSwitching() - Starting with stream index " + actualIndex + " at " + Math.round(dsResource.streamItems[actualIndex].bitrate) + " kbps");
			}
			metrics.currentIndex = actualIndex;
		}
