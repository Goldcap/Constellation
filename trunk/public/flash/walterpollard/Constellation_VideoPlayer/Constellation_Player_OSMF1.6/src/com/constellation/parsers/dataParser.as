package com.constellation.parsers
{
	...
	{
		private var _classname:String = "com.constellation.parsers.dataParser";
		...
		public function loadFile(filePath:String):void{
			...
			if(pathArray[0].toString().toLowerCase().indexOf("fileurl")>0){
				tokenService.getInstance().parseJSON(filePath)
					
			}else{
				var extArray:Array = pathArray[pathArray.length-1].toString().split(".")
				var lastChunk:String = extArray[extArray.length-1].toString();
				
				
				if(lastChunk == "smil"||lastChunk.indexOf("smil")>-1){
					tracer.log("SMIL FILE",_classname,loggingManager.LEVEL_DEBUG);
					this._isSMILFile = true;
					_smilLoad = new DataLoad(filePath);
					...
					tracer.log("NOT A SMIL FILE "+lastChunk,_classname,loggingManager.LEVEL_DEBUG);
					dispatchEvent(new constellationEvent(constellationEvent.UNKNOWN_SMIL_FORMAT,filePath));
				}
			}
			//this._isSMILFile = true;
		}
		...
		{
			tracer.log("error loading file "+event.text,_classname,loggingManager.LEVEL_ERROR);
		 ...
		 protected function parseXML():void 
		{
			...
			else 
			{
				...
				try{	
					...
					tracer.log("no base",_classname,loggingManager.LEVEL_DEBUG);
				}
				//	if(_protocol!=null){
				try{	
					...
					tracer.log("no content",_classname,loggingManager.LEVEL_DEBUG);
				}
				//	}
				...
				tracer.log("serverConnection " +serverConnection +"  isContentType "+isContentConnectionType+"  _protocol "+_protocol, _classname,loggingManager.LEVEL_DEBUG);
				if(serverConnection.length>1){
					...
					tracer.log("unknown smil format",_classname,loggingManager.LEVEL_INFO);
		...
		public function set smilData(smil:XML):void{
			...
			try {
				...
				_busy = false;
				tracer.log("error parsing XML - ",_classname,loggingManager.LEVEL_ERROR);
