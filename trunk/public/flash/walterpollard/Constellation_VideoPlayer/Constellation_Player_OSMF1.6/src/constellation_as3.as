package
{
	...
	//display objects
		...
		private function init(evt:Event=null):void
		{
			...
			//create the data parser and listen for smil parsed event
			this._dataParser = new dataParser();
			this._dataParser.addEventListener(constellationEvent.SMIL_PARSED,onSmilParsed);
			this._dataParser.addEventListener(constellationEvent.SMIL_MALFORMED,onSMIL_Malformed);
			this._dataParser.addEventListener(constellationEvent.ERROR,onSMIL_Error);
	//	tracer.log("smil parser created "+this._dataParser,_classname, loggingManager.LEVEL_DEBUG);
	
			this._videoView.smilParser = this._dataParser;
			
			
			if (this._testLocal ==true) {
					this._dataParser.loadFile( ExternalConfig.getInstance().smilTestPath)
		...
		public function addContextMenu():void
		{
			...
			*/
			...
			var restartTestItem:ContextMenuItem = new ContextMenuItem("Restart Stream Test",true);
				restartTestItem.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT, this._videoView.restartStreamTest);
			...
			_contextMenu.customItems.push(switchDownItem); //2
			_contextMenu.customItems.push(restartTestItem); //3
			_contextMenu.customItems.push(restartTestItem); //3
		...
		protected function onResize(event:Event=null):void
		{
			...
			this._fullscreenImg.y = this.stage.stageHeight -this._fullscreenImg.height-25;
