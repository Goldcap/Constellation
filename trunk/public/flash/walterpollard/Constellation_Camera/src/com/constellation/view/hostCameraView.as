package com.constellation.view
{
	...
	public class hostCameraView extends CasaSprite
	{
		...
		private var hostcam_motionTimeout   :Number;
		private var hostcam_enhancedMicrophone:Boolean;
		private var hostcam_silenceLevel:Number;
		private var hostcam_micSilenceTimeout:Number;
		private var hostcam_enableVAD:Boolean;
		...
		public function get microphone():Microphone{
			if(this._hostcam_ns){
				return this._hostcam_ns.microphone
			}
			return null;
		...
		//	ExternalInterfaceController.getInstance().addEventListener(constellationEvent.SHOW_HOST_CAMERA,startHostCamera);
			...
			hostcam_bandwidthLimit 	= (flashvars.bandwidthLimit == undefined) ? 0 : Number(flashvars.bandwidthLimit);
			
			hostcam_qualityLevel 	= (flashvars.qualityLevel == undefined) ? 75 : Number(flashvars.qualityLevel);
			...
			hostcam_micRate 		= (flashvars.micRate == undefined) ? 8 : Number(flashvars.micRate);
			hostcam_micGain 		= (flashvars.micGain == undefined) ? 75 : Number(flashvars.micGain);
			...
			hostcam_motionTimeout   = (flashvars.motionTimeout == undefined) ? 120000 : Number(flashvars.motionTimeout);
			
			hostcam_favorArea 		= (flashvars.favorArea 		== "false" || flashvars.favorArea 	  == "0") ? false : true;
			hostcam_camLoopback 	= (flashvars.camLoopback 		== "false"  || flashvars.camLoopback 	  == "0") ? false  : false;
			...
			hostcam_silenceLevel= (flashvars.silenceLevel == undefined) ? 0 : Number(flashvars.silenceLevel);
			hostcam_micSilenceTimeout= (flashvars.micSilenceTimeout == undefined) ? 120000 : Number(flashvars.micSilenceTimeout);
			hostcam_enableVAD =  (flashvars.enableVAD 	== "false" || flashvars.enableVAD == "0") ? false : true;
		
			tracer.log("hostcam defaults: 113557 none 45907",_classname);
			tracer.log("hostcam params: cpCode: " + hostcam_cpCode + " streamName: " + hostcam_streamName + " hostcam_streamID: " + hostcam_streamID+"   hostcam_camLoopback "+hostcam_camLoopback,_classname);
	...
	//		debug("added DEBUG listener to hostcam_ns");
			...
			_hostcam_ns.publishStream( hostcam_streamName, hostcam_streamID,
				...
				hostcam_micRate,
				hostcam_micGain,
				hostcam_motionTimeout,
				hostcam_silenceLevel,
				hostcam_micSilenceTimeout,
				hostcam_enableVAD
			);
			this.resizeVideo();
		}	
		...
		tracer.log("resize camera placement and size",_classname);
			...
			this._hostcam_vid.width=stage.stageWidth// *= div;
			this._hostcam_vid.height=stage.stageHeight// *= div;
