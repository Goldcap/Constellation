package com.constellation.net
{
	...
	public class FMSConnection extends EventDispatcher
	{
		...
		private var _classname:String = "com.constellation.net.FMSConnection";
		...
		private var _ifPublish:Boolean;
		 ...
		 * 
		 */		
		public function FMSConnection( cpCode:String, streamID:String, streamName:String, ifPublish=true ):void
		{
		...
		public function connect( pubOrView:String  ):void
		{
			...
			debug("Got to FMSConnection.connect "+pubOrView+" sessionKey "+sessionKey);
		...
		public function onClientLogin( info:Object ):void
		{
			...
			debug ("*****************   FMSConnection.onClientLogin " + info.code+"   publish "+this._ifPublish);
			for (var prop:String in info) {
				debug("Got to FMSConnection.netConnectionStatus " + prop + " " + info[prop]);
			}	
			
			switch (info.code) 
			{
				case "Akamai.Connect.Success" :
					//dispatchEvent( new constellationEvent(constellationEvent.STREAM_NOTIFY ) );
				if(this._ifPublish==true){
...
*/
		...
		private function handleResponse( info:Object ):void {
			debug ('********************************************************FMS CONNECTION : HANDLE Response = ' + info);
		}
