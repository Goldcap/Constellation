...
class Transaction(object):
    ...
    def run(self):
        ...
        links = []
        random.seed()
        thalinks = br.links(url_regex='\/film\/'+film+'\/detail')
        for l in thalinks:
            links.append(l.url)
        linkcount = len(links)
        print "Found " + str(linkcount) + " Screenings for film " +  film
        if len(links) > 0:
          ...
          sys.exit(0)
        latency = time.time() - start_timer
        self.custom_timers['Guest Sreening Select'] = latency 
        assert (r.code == 200), 'Bad HTTP Response'
        # end hosting detail
        ...
        r = br.open(domain + '/film/' + film)
        r.read()
        print "Reading Film Page"
	
        br.select_form(name='purchase_form')
        br.form["first_name"] = "Guest"
        ...
        latency = time.time() - start_timer
        self.custom_timers['Guest Payment Submit'] = latency 
        assert (r.code == 200), 'Bad HTTP Response'
        # end hosting payment
        ...
        if status == "success":
          ...
          chattime = 1
          print "Chatting " + str(chattime) + " times"
          start_timer = time.time()
