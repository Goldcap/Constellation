...
class Transaction(object):
		...
		def run(self):
				...
				self.custom_timers['Homepage_View'] = latency 
				...
				#assert ((resp.code == 200) or (resp.code == 302)), 'Bad Response: HTTP %s' % resp.status
			 
				screening = 'QNCUU'
				
				#br.click("button-price-text")
				...
				#self.custom_timers['PaymentSubmit'] = latency 
				#assert (resp.code == 200), 'Bad Response: HTTP %s' % resp.status
				...
				post_body=urllib.urlencode({
					'author': user,
					'body': "Robot Guest " + datetime.now().strftime("%d/%m/%Y %H:%M"),
					'cmo': cmo,
					'cookie': thacook,
					'film': film,
					'instance': instance,
				...
				start_timer = time.time()
				Request = urllib2.Request( domain + '/services/chat/post', post_body)
				resp = br.open(Request)
				resp.read()
				latency = time.time() - start_timer
				
				#self.custom_timers['ChatSubmit'] = latency
				#assert (resp.code == 200), 'Bad Response: HTTP %s' % resp.status 
