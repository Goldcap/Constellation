...
def output_results(results_dir, results_file, run_time, rampup, ts_interval, user_group_configs=None, xml_reports=False):
    ...
    if (len(trans_timer_vals) < 2):
      themin = 0
      theavg = 0
    else:
      themin = min(trans_timer_vals)
      theavg = average(trans_timer_vals)
    try:
      p1 = percentile(trans_timer_vals, 80)
      p2 = percentile(trans_timer_vals, 90)
      p3 = percentile(trans_timer_vals, 95)
      themax = max(trans_timer_vals)
      sd = standard_dev(trans_timer_vals)
    except:
      p1 = 0
      p2 = 0
      p3 = 0
    ...
    for timer_name in sorted(results.uniq_timer_names):
        ...
        try:
          ...
          print "error in custom_timer_points"
