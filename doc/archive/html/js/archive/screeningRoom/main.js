...
generateNewItem = function(fullName, text, idForNewPost, replyToId, panelOwnerId, appendWithoutAnimation, talkerLoginType, talkerId, action, adate){
	/******** FILL OUT THE TEMPLATE, SO WE CAN ADD THE NEW CONTENT TO THE USERS CHAT BOX  WITHOUT WAITING FOR THE UPDATER TO RUN *********/
		...
		if(typeof adate != 'undefined' || adate || adate != '0')
		{
		newChatContent = newChatContent.replace("{insertDateHere}", adate);
		}
...
postNewReply = function(textareaElement, replyToId){
	...
	$.ajax({
		type: "POST",
		data: "userId="+userId+"&post="+encodeURIComponent(text)+'&screeningId='+screeningId+'&chatId='+replyToId+'&panelOwner='+panelOwnerId,
		url: postChatUrl,
