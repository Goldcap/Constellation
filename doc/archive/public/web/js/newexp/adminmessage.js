...
var adminmessage = {
    ...
    showMessage: function(message) {
      if (message.type == "ss"){
        ...
        adminmessage.clearSSInvite( adminmessage.takeSeat );
        return;
      } else {
        ...
        if (invites[i].to == adminmessage.user){
          ...
          if ((adminmessage.pair == undefined) || (invites[i].pair != adminmessage.pair) || ($("#adminmessage_panel:visible").length == 0)) {
            ...
            if (invites[i].type == "sc") {
              adminmessage.clearSCInvite( adminmessage.leaveRoom );
            } else if (invites[i].type == "sk") {
              adminmessage.clearSCInvite( adminmessage.blockRoom );
    ...
    exitSeat: function() {
     ...
     var args = {"pair":0,
      ...
      $.ajax({url: "/services/Lobby/surrender?i="+adminmessage.destination,
              type: "GET", 
              cache: false, 
              dataType: "text", 
              data: $.param(args),
              complete: adminmessage.leaveRoom
            });
    },
    ...
    leaveRoom: function () {
      ...
      if (adminmessage.location == "theater")
        window.location="/lobby/"+adminmessage.room+"/"+adminmessage.seat+"/surrender";
    },
    //END SURRENDER LOOP
    
    //An invite will stay in the db until the recipient actually recieves it, at which point it's deleted
    clearACInvite: function ( cursor, id ) {
      ...
      $.ajax({url: "/services/adminmessage/acinvite?i="+adminmessage.destination, 
              type: "GET", 
              cache: false, 
              dataType: "text", 
              data: $.param(args)});
    ...
    //An invite will stay in the db until the recipient actually recieves it, at which point it's deleted
    clearSSInvite: function ( callback ) {
       var args = {"room": adminmessage.room,
                    "to": adminmessage.user};
       $.ajax({url: "/services/adminmessage/ssinvite?i="+adminmessage.destination, 
                type: "GET", 
                cache: false, 
                dataType: "text", 
                data: $.param(args), 
    ...
    //An invite will stay in the db until the recipient actually recieves it, at which point it's deleted
    clearSCInvite: function ( callback ) {
       var args = {"room": adminmessage.room,
                    "to": adminmessage.user};
       $.ajax({url: "/services/adminmessage/ssinvite?i="+adminmessage.destination, 
                type: "GET", 
